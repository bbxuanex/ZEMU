...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw, I, word_t val = Mr(src1 + imm, 4);
          // 打印看看究竟读了哪里！
          if (val == 0) { Log("LW Suspicious: addr=0x%x, val=0x%x, src1=0x%x, imm=0x%x", src1 + imm, val, src1, imm); } R(rd) = Mr(src1 + imm, 4));
  ...
  INSTPAT("010000 ?????? ????? 101 ????? 00100 11", srai, I, R(rd) = (sword_t)src1 >> (imm & 0x1f));
