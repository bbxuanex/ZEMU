...
{
  size_t ret = _syscall_(SYS_write, fd, (intptr_t)buf, (intptr_t)count);
  return ret;
}
...
{
  size_t ret = _syscall_(SYS_read, fd, (intptr_t)buf, (intptr_t)count);
  return ret;
}
...
{
  size_t ret = _syscall_(SYS_lseek, fd, (intptr_t)offset, whence);
  return ret;
}
