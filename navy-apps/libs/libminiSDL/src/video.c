...
void SDL_BlitSurface(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst, SDL_Rect *dstrect)
{
  ...
  for (int row = 0; row < h; row++)
  {
    ...
    uint8_t *src_row = src->pixels + (sy + row) * src->pitch + sx * bpp;
    uint8_t *dst_row = dst->pixels + (dy + row) * dst->pitch + dx * bpp;
...
void SDL_UpdateRect(SDL_Surface *s, int x, int y, int w, int h)
{
  ...
  if (x == 0 && w == s->w)
  {
    uint32_t *src = (uint32_t *)(s->pixels + y * s->pitch);
  ...
  for (int row = 0; row < h; row++)
  {
    uint8_t *src_row = s->pixels + (y + row) * s->pitch + x * 4;
    uint8_t *dst_row = (uint8_t *)buf + row * w * 4;
